
# Pre-Launch Checklist — Make It Media

Work through top to bottom. Check off as you go.

---

## 🟢 Phase 1 — Must-Have Before First Social Post

### Email Platform + Delivery
- [x] Choose email platform (ConvertKit/Kit, MailerLite, or Beehiiv) — **Kit free plan**
- [ ] Connect domain / verify sender email
- [ ] Set up brand template (logo, colours, footer)
- [x] Build lead magnet delivery email (instant auto-send on signup) — **Kit incentive email live**
- [~] Write welcome sequence — 3–5 emails over 7–14 days (blocked: Kit free doesn't support sequences; upgrade at ~50 subs or draft as broadcasts)
  - [x] Email 1: Deliver lead magnet + set expectations — **incentive email handles this**
  - [ ] Email 2: Story / credibility
  - [ ] Email 3: Value / quick win
  - [ ] Email 4: Case study or social proof
  - [ ] Email 5: Soft CTA to book strategy call

### Lead Magnets + Landing Pages
- [~] Decide on 1–3 higher-ticket lead magnets (topics + formats) — **#1 done, #2 and #3 not yet chosen**
- [x] Create lead magnet 1 (PDF/guide/template) — **"5 Website Mistakes Costing Health Practitioners Clients", 10-page PDF**
- [ ] Create lead magnet 2
- [ ] Create lead magnet 3
- [x] Build landing page 1 on site (form connected to email platform) — **/health-practitioners/guides/website-mistakes, Kit form embedded + exit-intent modal**
- [ ] Build landing page 2
- [ ] Build landing page 3
- [x] Test: submit form → email arrives → magnet delivers — **verified end-to-end**

### ManyChat + Social DM Automation
- [ ] Set up ManyChat account + connect Instagram
- [ ] Build comment-to-DM flow for lead magnet 1
- [ ] Build comment-to-DM flow for lead magnet 2
- [ ] Build comment-to-DM flow for lead magnet 3
- [ ] Flow captures email before sending magnet
- [ ] LinkedIn DM template (manual or via Aware — ManyChat doesn't integrate directly)
- [ ] Test each keyword trigger end-to-end

### Booking System
- [ ] Set up Calendly / SavvyCal / Cal.com
- [ ] Connect Google Calendar
- [ ] Configure strategy call event (duration, buffer, availability)
- [ ] Add intake questions to booking form
- [ ] Set up confirmation + reminder emails
- [ ] Embed on website (contact page + CTAs)
- [ ] Add link to email signature
- [ ] Add link to welcome sequence final email

### CRM / Lead Tracking
- [ ] Set up Airtable or Notion lead database
- [ ] Fields: name, email, source, lead magnet, date, status, notes
- [ ] Connect email platform → CRM (Zapier/native)
- [ ] Connect booking system → CRM

### Analytics + Tracking
- [ ] GA4 installed + firing on all pages
- [ ] Event tracking on all CTAs (use existing tracking IDs per CLAUDE.md)
- [ ] Meta Pixel installed
- [ ] LinkedIn Insight Tag installed
- [ ] UTM link template created for social posts
- [ ] Conversion events configured in GA4 (form submit, booking)

---

## 🟡 Phase 2 — Month 1–2 (After Posting Starts)

- [ ] Payment system — only if productising (Stripe or Dubsado/HoneyBook)
- [ ] Client onboarding automation (forms, contracts, welcome pack)
- [ ] Retargeting audiences built (Meta + LinkedIn)
- [ ] Testimonial collection system (Senja, or simple form + process)
- [ ] Second lead magnet sequence live
- [ ] A/B test headlines on top-performing landing page

---

## 🔴 Later — Don't Build Yet

- HubSpot paid / ActiveCampaign full suite
- Website chatbot
- Affiliate / referral program

---

## LinkedIn Optimisation (Parallel Track)

### Personal Profiles (You + Dad)
- [ ] Professional headshot uploaded
- [ ] Branded banner (1584x396px) with tagline + CTA
- [ ] Headline rewritten (what you do | who you help | outcome)
- [ ] Custom URL set
- [ ] Location + industry correct
- [ ] "Open to providing services" configured
- [ ] About section — hook in first 3 lines, proof, keywords, CTA
- [ ] Featured section — 3–6 pinned items with custom thumbnails
- [ ] Experience descriptions written (outcomes, not duties)
- [ ] Top 3 skills pinned + endorsements requested
- [ ] 3+ recommendations given, 3+ received
- [ ] Education, licenses, volunteer filled in

### Make It Media Company Page
- [ ] Logo 300x300px uploaded
- [ ] Cover banner 1128x191px on-brand
- [ ] Tagline written (120 chars)
- [ ] About section with keywords
- [ ] Website, industry, size, HQ, founded filled
- [ ] Specialties — 20 keyword tags
- [ ] Custom URL set
- [ ] Products/Services tab populated
- [ ] CTA button configured
- [ ] Pinned post set
- [ ] Employees associated with page

### Austplan Company Page
- [ ] Logo uploaded
- [ ] Cover banner on-brand
- [ ] Tagline written
- [ ] About section with keywords
- [ ] Website, industry, size, HQ, founded filled
- [ ] Specialties — 20 keyword tags
- [ ] Custom URL set
- [ ] Products/Services tab populated
- [ ] CTA button configured
- [ ] Pinned post set
- [ ] Employees associated with page

### Cross-Check
- [ ] Personal profiles link to company pages (role tagged)
- [ ] Messaging consistent across personal ↔ company
- [ ] All website links live + correct
- [ ] Mobile render checked
- [ ] SSI score baseline noted (linkedin.com/sales/ssi)

---

## 🚀 Launch Gate

Only start posting content when all of **Phase 1** is ✅ and LinkedIn profiles + company pages are ✅.
